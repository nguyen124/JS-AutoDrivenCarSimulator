define(['Class', 'Genome', 'Util'], function (Class, Genome, Util) {
	...
	var GeneticAlgorithm = Class.extend({
		...
		getNextGenome: function () {
			...
			return this.population.get(this.currentGenome);
		},
		...
		breedPopulation: function () {
			...
			crossedOverGenomes = this.crossOver(bestGenomes[0, bestGenomes[2]);
						...
						this.mutate(crossedOverGenomes[0]);
						this.mutate(crossedOverGenomes[1]);
						children.add(crossedOverGenomes[0]);
						children.add(crossedOverGenomes[1]);
						// For the remainding n population, add some random genomes.
						var remainingChildren = (this.totalPopulation - children.size());
						for (var i = 0; i < remainingChildren; i++) {
							children.push(this.createNewGenome(bestGenomes[0].weights.size()));
						}
						this.clearPopulation();
			...
			createNewGenome = function (totalWeights) {
				...
				for (var j = 0; j < totalWeights; j++) {
					genome.weights.add(Util.RandomSigmoid());
				}
				this.genomeID++;
				return genome;
			},
			...
			crossOver: function (g1, g2) {
				...
				var crossover = Math.abs(Math.random() * Number.MAX_VALUE) % totalWeights;
				var genomes = [];
				...
				for (var i = crossover; i < totalWeights; i++) {
					genome1.weights.add(g2.weights[i]);
					genome2.weights.add(g1.weights[i]);
				}
				genomes.add(genome1);
				genomes.add(genome2);
				return genomes;
				...
				while (genomeCount < totalGenomes) {
					...
					for (var i = 0; i < this.totalPopulation; i++) {
						if (this.population[i].fitness > bestFitness) {
							var isUsed = false;
							for (var j = 0; j < bestGenomes.size(); j++) {
								if (bestGenomes[j].ID == population[i].ID) {
									isUsed = true;
								}
							}
							if (isUsed == false) {
					...
					if (bestIndex != -1) {
						genomeCount++;
						bestGenomes.add(this.population[bestIndex]);
					}
				}
				return bestGenomes;
			},
